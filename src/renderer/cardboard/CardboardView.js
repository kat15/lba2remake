...
export function CardboardView(screen_params, device_params) {
    this.screen = screen_params;
    this.device = device_params;
}

CardboardView.prototype = {
    ...
    getLeftEyeFov: function () {
        ...
        var bottomDist = getYEyeOffsetMeters(screen, cdp);
    ...
    getLeftEyeFovAndViewportNoDistortionCorrection: function () {
        ...
        var eyePosY = getYEyeOffsetMeters(screen, cdp) / eyeToScreenDist;
...
Object.defineProperties(CardboardView.prototype, {
    device: {
        get: function () {
            return this._device;
        },
        set: function (value) {
            this._device = value;
            this.distortion = new DistortionParams(
...
const ScreenParams = function (width, height, dpi, border_size_meters) {
    this.width = width;
    this.height = height;
    this.dpi = dpi;
...
Object.defineProperties(ScreenParams.prototype, {
    width_meters: {
        get: function () {
...
function getYEyeOffsetMeters(screen_params, device_params) {
...
function DistortionParams(coefficients) {
    this.coefficients = coefficients;
}

DistortionParams.prototype = {
...
export function getProjectionMatrixPair(left_fov_angles, near, far) {
...
function updateBarrelDistortion(barrel_distortion, cardboard_view,
    ...
    var projections = getProjectionMatrixPair(cardboard_view.getLeftEyeFov(), camera_near, camera_far);
    ...
    var unprojections = getProjectionMatrixPair(no_lens_view.fov, camera_near, camera_far);
...
export function findScreenParams() {
    var ppi, device_name;
    var ppi_entry = SCREEN_PPI_BY_DEVICE[WURFL.complete_device_name];
    if (ppi_entry) {
        ppi = ppi_entry[0];
        console.log('Detected', WURFL.complete_device_name);
    } else {
        // try regex match
        for (device_name in SCREEN_PPI_BY_DEVICE) {
            ppi_entry = SCREEN_PPI_BY_DEVICE[device_name];
    ...
    if (WURFL.is_mobile) {
        ...
        return new ScreenParams(screen_width, screen_height, ppi,
            0.003 /*bezel height*/);
    }
    // generic values for desktop
    return new ScreenParams(1920, 1080, 445, 0);
}
